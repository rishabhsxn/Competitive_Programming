...
/* Calculate sum of every possible pairs between the two arrays,
   calculate the highest sum which is smaller than budget.
   Take a flag variable intially 0, while Calculating sums, if any of the
   sum is smaller than budget, make the value of flag=1, than means she can
   buy atleast one pair set, however, if at the end of calculating sums
   if flag is 0, then return -1 */

int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b)
{
   int flag=0,sum=0,max_sum=0;
